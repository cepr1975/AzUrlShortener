...
{
    public class LSUrlDeleteExpired
    {
        private readonly ILogger _logger;
        private readonly ShortenerSettings _settings;

        public LSUrlDeleteExpired(ILoggerFactory loggerFactory, ShortenerSettings settings)
        {
            _logger = loggerFactory.CreateLogger<UrlList>();
            _settings = settings;
        }

        [Function("LSUrlDeleteExpired")]
        public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "api/LSUrlDeleteExpired")] HttpRequestData req,
                                    ExecutionContext context
                                )
        {
            _logger.LogInformation($"HTTP trigger - LSUrlDeleteExpired");
